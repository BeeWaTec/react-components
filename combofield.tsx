...
export default function ComboField({
  ...
  useEffect(() => {
    ...
    props.loadValuesPath?.filter,
    props.loadValuesPath?.sort,
    props.loadValuesPath?.limit,
    props.loadValuesPath?.offset,
  ]);
  ...
  return (
    ...
    >
      ...
      >
        <Combobox.Button className={`block w-full h-full`}>
          <Combobox.Input
        ...
        <Combobox.Button
          className={`inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none ${props.disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
        >
        ...
        {values && values.length > 0 && (
          <Combobox.Options
            static={showDropdown}
            className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm ml-0"
          >
            {values.map((value) => (
              <Combobox.Option
                key={value.value}
