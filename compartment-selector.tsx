...
const CompartmentSelector = forwardRef<HTMLDivElement, CompartmentSelectorProps>(
      ...
      >
        ...
        {options
          .filter((item) => {
            ...
            while (true) {
              const areaRegex =
                /(bag|box_lid|box_body|compartment_frame|compartment_partition|compartment_bottom):\s*(true|false)/i;
            ...
            while (true) {
              ...
              if (weightThicknessRegex.test(_search)) {
                ...
                console.log(
                  `Regex: ${weightThicknessRegex} -> Min/Max: ${minMax}, Weight/Thickness: ${weightThickness}, Value: ${value}, Unit: ${unit}`,
                );
                ...
                console.log(
                  `Item: ${item.weightMin} - ${item.weightMax} - ${item.thicknessMin} - ${item.thicknessMax}`,
                );
                if (minMax === "min" && weightThickness === "weight" && item.weightMin < Number.parseFloat(value))
                  return false;
                if (minMax === "max" && weightThickness === "weight" && item.weightMax > Number.parseFloat(value))
                  return false;
                if (minMax === "min" && weightThickness === "thickness" && item.thicknessMin < Number.parseFloat(value))
                  return false;
                if (minMax === "max" && weightThickness === "thickness" && item.thicknessMax > Number.parseFloat(value))
                  return false;
              } else break;
            }
            ...
            while (true) {
              const attributeRegex =
                /(component-protection|endurance|clip-resistance|sliding|surface-stability):\s*(\d+)/i;
              if (attributeRegex.test(_search)) {
                ...
                if ((item as any)[attribute] < Number.parseFloat(value)) return false;
              } else break;
            }
          ...
          .map((item) => (
            <CompartmentCard key={item.id} item={item} isSelected={item.id === value} onSelect={handleSelect} />
          ))}
      </div>
    );
  },
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col">
        <div className="relative text-xl font-bold w-full shadow-md h-12 flex flex-row flex-nowrap items-center justify-center">
          {item.colors.length > 0 && (
            ...
            >
              <div className="absolute top-0 left-0 right-0 bottom-0 z-0">
                <img src={TransparentPattern.src} className="absolute inset-0 w-full h-full object-cover z-0" alt={``} />
              </div>
              <div
        ...
        <div className="relative text-xl font-bold w-full shadow-md h-4 flex flex-row flex-nowrap items-center justify-center">
        ...
        <div className="flex flex-col p-4">
          <div className="flex flex-row items-center">
            ...
            {item.weightMin !== item.weightMax ? (
              <span>
                {item.weightMin} - {item.weightMax} {item.weightUnit || "g/m²"}
              </span>
            ) : (
              <span>
                {item.weightMin} {item.weightUnit || "g/m²"}
              </span>
            )}
          </div>
          <div className="flex flex-row items-center">
            ...
            {item.thicknessMin !== item.thicknessMax ? (
              <span>
                {item.thicknessMin} - {item.thicknessMax} mm
              </span>
            ) : (
        ...
        <div className="italic h-14 p-2 overflow-hidden overflow-ellipsis w-full">
          {item.otherFeatures.length > 0 && <>{item.otherFeatures.join(", ")}</>}
        </div>
        <div className="flex flex-row gap-2 p-2 w-full h-18 text-sm flex-wrap overflow-hidden">
          {/* As bag */}
          {item.areasOfUse.bag && (
            <div className="p-1 bg-gray-100 text-gray-500 text-center no-wrap whitespace-nowrap">Bag</div>
          )}
          {/* As box lid */}
          {item.areasOfUse.box_lid && (
            <div className="p-1 bg-gray-100 text-gray-500 text-center no-wrap whitespace-nowrap">Box Lid</div>
          )}
          {/* As box body */}
          {item.areasOfUse.box_body && (
            <div className="p-1 bg-gray-100 text-gray-500 text-center no-wrap whitespace-nowrap">Box Body</div>
          )}
          {/* As compartment frame */}
          {item.areasOfUse.compartment_frame && (
            <div className="p-1 bg-gray-100 text-gray-500 text-center no-wrap whitespace-nowrap">Compartment Frame</div>
          )}
