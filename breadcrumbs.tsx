            ...
            <ol role="list" className="flex space-x-4 rounded-md bg-white px-6 shadow">
                    ...
                    <div className="flex items-center justify-center h-full text-gray-400">
                        {!homeHref || homeHref == null || /^https?:\/\/((?:[\w\d-]+\.)+[\w\d]{2,}).*$/.test(homeHref) ?
                            ...
                            <Link
                                href={homeHref}
                                className="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700"
                                aria-current={typeof window !== 'undefined' && window.location.pathname == homeHref ? 'page' : undefined}
                            >
                                <HomeIcon className="h-5 w-5 flex-shrink-0" aria-hidden="true" />
                                <span className="sr-only">Home</span>
                            </Link>
                ...
                {showLoading &&
                    <li className="flex">
                        ...
                        <Spinner
                            className="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 w-7 h-7 self-center"
                        />
                    </li>
                }
                {pages.filter(page => page !== null && page.visible !== false).map((page) => (
